...
const FaqSection = () => {
  ...
  const faqs = [
    ...
    },
    {
      question: "You apply to jobs for me?",
      answer:
        "Yes, we do! Our automated system applies to relevant jobs on your behalf, saving you countless hours of manual applications. We ensure each application is tailored to the specific role and company.",
    },
    {
      question: "How long does it take to get a job with this program?",
      answer:
        "While results vary based on individual circumstances, most of our participants secure interviews within 2-4 weeks and job offers within 1-3 months. Our comprehensive approach significantly reduces the typical job search timeline.",
    },
    {
      question: "What does Persist Ventures get?",
      answer:
        "We take a small percentage of your first year's salary increase as our success fee. This means we only succeed when you succeed. Our interests are perfectly aligned - the more we help you earn, the better it is for both of us.",
    },
  ];
  ...
  const eyeSvg = (
    <svg
      viewBox="0 0 24 24"
      className="w-[20px] h-[20px] fill-white opacity-90 transition-all duration-300"
    >
      <path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z" />
    </svg>
  );
  ...
  return (
    <div
      className="faq-section max-w-[1500px] mx-auto mt-30 px-5 text-center lg:px-10 xl:px-20"
      id="faq"
    >
      <h2 className="text-[2.5rem] md:text-[3.5rem] font-extralight text-white mb-5 leading-[1.1] flex flex-col gap-2.5">
        <span>Curious? We've Got</span>
        <span className="font-medium">Answers!</span>
      </h2>
      <div className="faq-grid flex flex-col max-w-[1200px] mx-auto rounded-[24px] overflow-hidden relative">
        {faqs.map((faq, index) => (
          <div
            className="faq-item relative border-b border-[rgba(255,255,255,0.1)]"
            key={index}
          >
            <div
              className={`faq-question px-6 md:px-8 py-8 md:py-10 flex items-center justify-between cursor-pointer gap-5 relative transition-all duration-300 ${
                activeIndex === index ? "bg-[rgb(151,135,234)]" : ""
              }`}
              onClick={() => toggleFaq(index)}
            >
              <h3 className="text-[1rem] md:text-[1.1rem] font-normal text-[rgba(255,255,255,0.9)] text-left m-0 flex-1 leading-[1.4] flex items-center">
                <span className="mr-3.75 text-[1rem] md:text-[1.1rem] font-medium text-[rgba(255,255,255,0.5)] opacity-70">
                  0{index + 1}
                </span>
                {faq.question}
              </h3>
              <div className="faq-icon w-[28px] md:w-[32px] h-[28px] md:h-[32px] bg-[rgba(255,255,255,0.15)] rounded-full flex items-center justify-center transition-all duration-300 flex-shrink-0">
                {activeIndex === index ? eyeSlashSvg : eyeSvg}
              </div>
            </div>
            <div
              className={`faq-answer overflow-hidden transition-max-h duration-300 ${
                activeIndex === index
                  ? "max-h-[1000px] bg-[rgb(151,135,234)]"
                  : "max-h-0"
              }`}
            >
              <p className="px-6 md:px-8 py-4 md:py-6 pl-12 md:pl-18 m-0 text-[0.9rem] md:text-[1rem] text-[rgba(255,255,255,0.7)] leading-[1.6] text-left">
                {faq.answer}
              </p>
            </div>
          </div>
